...
##### CI Pipeline
The CI Pipeline for this project revolved around Jenkins and Git. Each time a new build has been pushed to the feature branch a jenkins user has been configured on the VM via editing the etc/sudoers file with the visudo command. this jenkins user is given full permissions and allows jenkins to run.
![jenkinsconfig](Documentation/config1.PNG)
![jenkinsconfig2](Documentation/config2.PNG)
By setting up the Jenkins build with these settings the jenkins server is constantly monitoring the GIT repo, once the branch is assigned within the jenkins configuration the jenkins build will constantly monitor the repository for pushes to the feature branch. Then using another config option, a hook trigger, the build is set to automatically run each time the feature branch has been pushed to.
...
![cipipeline](Documentation/cidiagram.PNG)
Pictured above is the continuous integration pipeline for the project. It illustrates the stages for the project and how the application seamlessly goes from devleopment to deployment (providing it passes the tests). The CI pipeline for this project utilized all the tools and techniques taught so far, starting with the sprint kanban board tech, coding solutions with python and utilizing imports/various libraries (specifically the database model layer and flask), testing the solutions to the code by writing unit tests and integration tests, using pytest and selenium. From here all the knowledge is compiled and a CI server is integrated to combine everything together, using Jenkins the entire process becomes more streamlined and optimized.
#### Project Tracking
